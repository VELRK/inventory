...
class Booking_model extends CI_Model
{
	...
	{
		return $this->db->where('id', (int) $id)->where('deleted_at IS NULL', null, false)->get('bookings')->row();
	}
	...
	public function list_filtered($filters, $limit, $offset)
	{
		...
		$rows = $this->db->order_by('b.id', 'DESC')->limit($limit, $offset)->get()->result();
		$items = array();
		foreach ($rows as $row) {
	...
	public function summary($filters)
	{
		...
		$sum = $this->_sum($filters);
		$customers = $this->db->select('COUNT(DISTINCT customer_name) as c', false);
		$this->_apply($filters);
		$cust = $this->db->get()->row();
		return array(
			'total' => (int) $total,
			'total_value' => (float) $sum,
			'total_customers' => $cust ? (int) $cust->c : 0
		);
	}

	private function _apply($filters)
	{
		$this->db->from('bookings b')->where('b.deleted_at IS NULL', null, false);
		if (!empty($filters['company_id'])) {
			$this->db->where('b.company_id', (int) $filters['company_id']);
		}
		if (!empty($filters['project_id'])) {
			$this->db->where('b.project_id', (int) $filters['project_id']);
		}
		...
		if (!empty($filters['from'])) {
			$this->db->where('b.booking_date >=', $filters['from']);
		}
		if (!empty($filters['to'])) {
			$this->db->where('b.booking_date <=', $filters['to']);
		}
		if (!empty($filters['q'])) {
			$this->db->group_start()->like('b.customer_name', $filters['q'])->or_like('b.customer_phone', $filters['q'])->group_end();
		}
	}
